...
async function renderShop() {
    ...
    let canUpgrade = nextUpgrade && coins >= nextUpgrade.cost && !savingsMaxed && !savingsLevelLimit;
    let savingsCard = `
      <div class="shop-card">
    ...
    if (nextUpgrade && canUpgrade) {
        document.getElementById('upgrade-savings-btn').onclick = async () => {
            ...
            if (freshLevel !== savingsLevel || freshCoins < nextUpgrade.cost || freshLevel >= freshCharLevel) {
                renderShop();
                return;
            }
